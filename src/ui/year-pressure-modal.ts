...
import { calculateReliefScore, RELIEF_ACTIVITY_DEFINITIONS, summarizePressureLedger } from "../analytics/pressure-ledger";
...
export class YearPressureModal extends Modal {
  /** 创建年度分享弹窗并注入本地数据 */
  constructor(app: App, private readonly records: AttendanceRecord[], private readonly reliefEntries: ReliefEntry[], private readonly today: string) {
    super(app);
  }
  ...
  onOpen(): void {
    ...
    /** 当年按日封顶抵扣后的压力账本 */
    const ledger = summarizePressureLedger(yearRecords, yearEntries, this.today);
    /** 弹窗中适合截图的内容卡 */
    ...
    RELIEF_ACTIVITY_DEFINITIONS.forEach((definition) => {
      ...
      rowEl.createSpan({ text: `${formatScore(calculateReliefScore(activityEntries))} 分` });
    });
